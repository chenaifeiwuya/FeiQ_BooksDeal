...
#define CLEAR_SELECT_WDG  (true)   //清空搜索窗口
#define NO_CLEAR_SELECT_WDG  (false)   //不清空
...
//p判断是否是学校的书籍
#define SCHOOL_BOOK (0)
#define NO_SCHOOL_BOOK (1)
#define OTHER_GOODS  (2)
#define ALL_BOOK_AND_OTHER   (3)   //所有物品，包括书籍和非书籍物品
#define SELECT_BOOK  (4)   //标志位，表示这个包是搜索书籍信息的请求包
#define MY_BOOK      (5)   //标志位，表示这个包是本机上架的书籍信息的请求包
...
#define NO_USER (-1)     //用于判断语音类是否有人使用
...
#define _DEF_IMG_PATH_SIZE (15)   //图片文件路径大小
...
#define SUBMIT_SUCCESS (2)    //上架成功
 ...
 struct STRU_TCP_BOOKS_INFO_RQ {
     STRU_TCP_BOOKS_INFO_RQ() : type(_DEF_PROTOCOL_TCP_BOOKS_INFO_RQ), num(0),isSchool(-1),isSelect(-1) {
         memset(bookName, 0, _DEF_BOOK_NAME_SIZE);
     }
